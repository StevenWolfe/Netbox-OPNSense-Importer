...
*   **`Could not retrieve interface name mapping; interface descriptions may be blank.`** (usually paired with a pair of HTTP 403s for `get_interface_names`/`getInterfaceNames`): the API user's group is missing **`Diagnostics: Logs: Firewall: Live View`** — see the permissions list above. Confirmed against OPNsense core's own ACL source: `api/diagnostics/interface/get_interface_names` is granted by that privilege and *no other*, and the legacy camelCase `getInterfaceNames` has no ACL pattern at all (short of `All Pages`), so both attempts 403 together. In practice this warning is usually cosmetic rather than a real data-loss signal: on OPNsense 23.1+ the primary data source (`/api/interfaces/overview/interfaces_info`) already returns its own per-interface `description` using the same fallback logic internally, so if your log shows interfaces synced "via Interfaces Overview API", descriptions are populated regardless of this warning. Grant the privilege anyway to silence the noise and to cover the legacy fallback path.
